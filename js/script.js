document.addEventListener("DOMContentLoaded", () => {
    ...
    ...(typeof productosComprimidos !== 'undefined' ? productosComprimidos : []),
    ...(typeof productosColageno !== 'undefined' ? productosColageno : []),
    ...
    const entreClienteInput = document.getElementById("entre-cliente");
    ...
    const cancelarDatosBtn = document.getElementById("cancelar-datos");
    ...
    confirmarDatosBtn.addEventListener("click", () => {
        ...
        const entre = entreClienteInput.value.trim();
        const direccion = direccionClienteInput.value.trim();

        if (!nombre || !ciudad || !calle || !entre || !direccion) {
        ...
        let mensaje = `*Pedido de ${nombre}*\n*Ciudad:* ${ciudad}\n*Dirección:* ${calle} ${direccion} *entre* ${entre}\n\n*Productos:*\n`;
    ...
    cancelarDatosBtn.addEventListener("click", () => {
        datosClienteModal.classList.remove("active");
    });
  ...
  if (!isNaN(precioNum)) {
    let precioAntes = Math.round(precioNum * 1.1);
